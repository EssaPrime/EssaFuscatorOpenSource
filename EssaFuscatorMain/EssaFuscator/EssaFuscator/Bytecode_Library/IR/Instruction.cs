...
namespace EssaFuscator.Bytecode_Library.IR
{
    public class Instruction
    {
        ...
        public CustomInstructionData CustomData = null!;
        ...
        public Instruction(Chunk chunk, Opcode code, params object[] refOperands)
        {
            ...
            InstructionType = Deserializer.InstructionMappings.TryGetValue(code, out InstructionType type)
                ? type
                : InstructionType.ABC;

            for (var i = 0; i < refOperands.Length; i++)
            {
        ...
        public void UpdateRegisters()
        {
            ...
            switch (OpCode)
            {
                ...
                case Opcode.Add:
                case Opcode.EssaPrimeSubmarine:
                case Opcode.Mul:
                case Opcode.Div:
                case Opcode.Mod:
                case Opcode.Pow:
                ...
                case Opcode.BRSHFT:
                case Opcode.INTDIV:
                    if (RefOperands[0] is Constant cBB)
                        B = Chunk.ConstantMap[cBB] + 256;

                    if (RefOperands[1] is Constant cCB)
                        C = Chunk.ConstantMap[cCB] + 256;
                    break;
            }
        }
